...
// 检查并设置 API key
async function checkAndSetApiKey(context: vscode.ExtensionContext) {
...
async function generateVerilog(prompt: string, context: vscode.ExtensionContext): Promise<string> {
    const apiKey = context.globalState.get('openaiApiKey');
    const model = context.globalState.get('openaiModel');
...
function packageCode(filePath: string) {
